...
class Nplus1News_DataSource @Inject constructor(
    @Named("NoCache") private val httpClient: OkHttpClient,
    @Named("Cache") private val httpClient_cache: OkHttpClient
):DataSource {
    companion object{
        const val base_url = "https://nplus1.ru"
        const val nameNewsCompany = "НПлюс"
    }
    ...
    override fun handleRssFile(response:Response,beginDate:Long){
        ...
        while (eventType != XmlPullParser.END_DOCUMENT) {
            if (parser.name=="item" && eventType == XmlPullParser.START_TAG) {
                ...
                tempMap.put(News_DTO.CATEGORY, "News")
                while(parser.name!="item"){
                    when(parser.name){
                        ...
                        News_DTO.LINK  -> tempMap.put(parser.name, parser.nextText()+"/amp")
                        "media:content" -> tempMap.put(News_DTO.LINK_IMAGE, parser.getAttributeValue(0))
                        News_DTO.DATE -> {
